...
class PreferencesView(ctk.CTkScrollableFrame):
    ...
    def apply_theme(self):
        ...
        if hasattr(self, "sec_env"):
            self.sec_env.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "sec_app"):
            self.sec_app.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "r_app"):
            self.r_app.configure(fg_color=ThemeManager.surface())
        if hasattr(self, "sec_dns"):
            self.sec_dns.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "r_dns"):
            self.r_dns.configure(fg_color=ThemeManager.surface())
        if hasattr(self, "sec_about"):
            self.sec_about.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "r_about"):
            self.r_about.configure(fg_color=ThemeManager.surface())
        if hasattr(self, "lbl_env_summary"):
            self.lbl_env_summary.configure(text_color=ThemeManager.text())
        if hasattr(self, "theme_var"):
            self.theme_var.set(ThemeManager.get_current_theme_key().capitalize())
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            self.r_app, text=tr("UI Scale / Font Size:"), font=Fonts.bold(12), text_color=ThemeManager.text()
        ...
        ctk.CTkLabel(
            self.r_app, text=tr("|  Theme Palette:"), font=Fonts.bold(12), text_color=ThemeManager.text()
        ...
        ctk.CTkLabel(self.r_app, text=tr("|  Language:"), font=Fonts.bold(12), text_color=ThemeManager.text()).pack(
            side="left", padx=(16, 6)
        )
    ...
    def import_dns_list(self):
        file_path = filedialog.askopenfilename(
            title="Pilih file DNS (.json atau .txt)", filetypes=[("DNS Database", "*.json *.txt"), ("All Files", "*.*")]
        )
        if not file_path:
            return
                ...
                for line in lines:
                    ...
                    if line and not line.startswith("#") and "," in line:
                        ...
                        if len(parts) >= 2:
                            ...
                            curr[p_id] = {"name": parts[0], "country": "🌐", "ipv4": parts[1:], "doh_url": ""}
                            imported_count += 1
                db.save_providers(curr)
    ...
    def import_dnsjumper_ini(self):
        file_path = filedialog.askopenfilename(
            title="Pilih file DnsJumper.ini", filetypes=[("DnsJumper Config", "*.ini"), ("All Files", "*.*")]
        )
        if not file_path:
            return
    ...
    def export_dns_list(self):
        ...
        try:
            ...
            self.main_app.show_toast(
                f"✓ Berhasil mengekspor {len(provs)} DNS ke {Path(file_path).name}", level="success"
            )
        except Exception as e:
