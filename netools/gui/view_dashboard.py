...
class DashboardView(ctk.CTkFrame):
    ...
    def apply_theme(self):
        ...
        if hasattr(self, "cards"):
            self.cards.configure(fg_color=ThemeManager.bg())
        if hasattr(self, "card_proxy"):
            self.card_proxy.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "card_pac"):
            self.card_pac.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "card_dns"):
            self.card_dns.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "card_9r"):
            self.card_9r.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "qa"):
            self.qa.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "btn_row"):
            self.btn_row.configure(fg_color=ThemeManager.surface())
        if hasattr(self, "pac_quick"):
            self.pac_quick.configure(fg_color=ThemeManager.surface(), border_color=ThemeManager.border())
        if hasattr(self, "pac_url_frame"):
            self.pac_url_frame.configure(fg_color=ThemeManager.surface())
        if hasattr(self, "pac_entry"):
            self.pac_entry.configure(
        ...
        if hasattr(self, "lbl_dns_stat"):
            self.lbl_dns_stat.configure(text_color=ThemeManager.text_muted())
        if hasattr(self, "lbl_9r_stat"):
            self.lbl_9r_stat.configure(text_color=ThemeManager.text_muted())
    ...
    def _build_ui(self):
        ...
        self.lbl_pac_stat = ctk.CTkLabel(
            self.card_pac, text=pac_init, font=Fonts.regular(12), text_color=ThemeManager.text_muted(), justify="left"
        )
        self.lbl_pac_stat.pack(anchor="w", padx=14, pady=(0, 12))

        # Card 3: DNS Resolver
        self.card_dns = ctk.CTkFrame(
            self.cards,
            fg_color=ThemeManager.surface(),
            corner_radius=8,
            border_width=1,
        ...
        ctk.CTkLabel(
            self.card_dns, text=tr("⚡ Active System DNS"), font=Fonts.subtitle(13), text_color=ThemeManager.warning()
        ).pack(anchor="w", padx=14, pady=(12, 4))
        ...
        ctk.CTkLabel(
            self.qa, text=tr("⚡ Quick 1-Click Operations"), font=Fonts.subtitle(13), text_color=ThemeManager.text()
        ...
        ctk.CTkLabel(self.pac_url_frame, text=tr("PAC URL:"), font=Fonts.bold(12), text_color=ThemeManager.text()).pack(
            side="left", padx=(0, 6)
        )
