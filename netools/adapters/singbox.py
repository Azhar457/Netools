...
from netools.config import CONFIGS_DIR, LOGS_DIR, PID_DIR, HTTP_PORT_OFFSET
...
def stop_all_singbox_instances() -> None:
    """Kill all active singbox instances recorded in PID_DIR."""
    for pid_file in PID_DIR.glob("*.pid"):
        name = pid_file.stem
        stop_singbox_instance(name)

    try:
        subprocess.run(["pkill", "-f", "sing-box run -c"], capture_output=True)
    except Exception:
        pass
